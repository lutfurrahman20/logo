"""
Manchester City FC — Player Headshot Downloader (Soccer / EPL)
...
Sources (tried in order per player):
  1. FPL CDN      — resources.premierleague.com/110x140 (current-season kit photos)
                    via Fantasy Premier League bootstrap API (team 13).
                    Covers ~22 established squad members.
  2. Transfermarkt — img.a.transfermarkt.technology portrait photos.
                    Fallback for players not yet in FPL CDN (new arrivals,
                    youth players).
  3. ESPN CDN      — espncdn.com headshots (very sparse, last resort).
...
# ── Config ─────────────────────────────────────────────────────────────────────
ESPN_ROSTER_API   = (
    "https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/teams/382/roster"
)
...
# 2025/26 season CDN — current-season kits, no 'p' prefix, covers recent transfers
PL25_CDN          = "https://resources.premierleague.com/premierleague25/photos/players/110x140/{code}.png"
# Legacy CDN — older photos, may show previous-club kits for recent transfers
FPL_CDN           = "https://resources.premierleague.com/premierleague/photos/players/110x140/p{code}.png"
MANCITY_FPL_ID    = 13

TM_KADER_URL      = "https://www.transfermarkt.com/manchester-city/kader/verein/281"
TM_PORTRAIT_CDN   = "https://img.a.transfermarkt.technology/portrait/medium/"

OUTPUT_DIR = Path("f:/logo/manchester_city_soccer_player_logos")
...
# Required where ESPN uses a nickname/shortened form that won't fuzzy-match FPL.
ESPN_TO_FPL_NAME = {
    # ESPN "Rodri" → FPL full name (1 shared word only, fuzzy fails)
    "Rodri":          "Rodrigo 'Rodri' Hernandez Cascante",
    # ESPN "Savinho" → FPL full name (no shared normalized words)
    "Savinho":        "Sávio Moreira de Oliveira",
    # Defensive extras that pass fuzzy but override ensures best kit photo
...
# Characters with no NFKD decomposition to ASCII must be mapped explicitly.
...
def build_espn_roster() -> list[dict]:
    """Returns list of {display_name, id} dicts from Manchester City's ESPN roster."""
    players = []
    try:
        print("  Fetching ESPN roster (team 382 / eng.1) ...")
...
def build_fpl_lookup() -> dict:
    """
    Returns {normalized_name: best_photo_url} for every Man City player in FPL.
    Prefers the 2025/26 premierleague25 CDN (current-season kits, covers recent
    transfers like Semenyo, Donnarumma, Guéhi, Aït-Nouri, Cherki).
    Falls back to the legacy FPL CDN for any code not yet on premierleague25.
    """
    lookup: dict = {}
    try:
        ...
        for p in r.json().get("elements", []):
            ...
            if not (full and code):
                continue
            # Prefer 2025/26 CDN (current-season kit); fall back to legacy CDN
            pl25_url = PL25_CDN.format(code=code)
            fpl_url  = FPL_CDN.format(code=code)
            lookup[normalize(full)] = pl25_url  # verified at download time
            lookup[normalize(full) + "__fallback"] = fpl_url  # internal fallback key
    except Exception as e:
...
def build_tm_lookup() -> dict:
    """
    Scrapes the Transfermarkt kader page for Man City and returns
    {normalized_name: portrait_url}.
    """
    lookup: dict = {}
    try:
...
def fpl_url_for(espn_display_name: str, fpl_lookup: dict) -> str | None:
    """
    Resolve ESPN display name → best photo URL.
    Tries premierleague25 CDN first (current-season kit), then legacy FPL CDN.
    Verifies image is real (HTTP 200, >5 KB).
    """
    ...
    for name in candidates:
        ...
        if matched:
            # Try PL25 CDN first, then legacy CDN
...
def tm_url_for(espn_display_name: str, tm_lookup: dict) -> str | None:
    """Resolve ESPN display name → Transfermarkt portrait URL. Verifies image is real."""
    candidates = [espn_display_name]
    override = ESPN_TO_FPL_NAME.get(espn_display_name)
    if override:
        candidates.append(override)
...
def main() -> None:
    ...
    print("=" * 60)
    print("  Manchester City FC — Player Headshot Scraper")
    print("=" * 60)
