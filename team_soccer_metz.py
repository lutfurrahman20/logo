"""
FC Metz — Player Headshot Downloader (Soccer / Ligue 2)

Sources (tried in order per player):
  1. ESPN CDN        — a.espncdn.com headshots
  2. Transfermarkt   — portrait photos (verein/347)
  3. FC Metz official — fcmetz.com squad API (full-body jersey + face photos)

Squad sourced from:
  https://www.espn.com/soccer/team/squad/_/id/177/metz
"""
...
ESPN_ROSTER_API  = "https://site.api.espn.com/apis/site/v2/sports/soccer/fra.2/teams/177/roster"
ESPN_HEADSHOT    = "https://a.espncdn.com/i/headshots/soccer/players/full/{id}.png"
TM_KADER_URL     = "https://www.transfermarkt.com/fc-metz/kader/verein/347"
METZ_SQUAD_API   = "https://www.fcmetz.com/fr/Squad/_Items"
METZ_IMG_BASE    = "https://www.fcmetz.com/Resource/GetTeamSeasonPlayerPicture"
METZ_SQUAD_MODEL = {"TeamId": 1, "SelectedSeasonYear": 2025, "WebPageId": 307, "DisplayPictures": True}
OUTPUT_DIR       = Path("f:/logo/metz_soccer_player_logos")

HEADERS = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"}
TM_HEADERS   = {**HEADERS, "Accept-Language": "en-US,en;q=0.9", "Referer": "https://www.transfermarkt.com/"}
METZ_HEADERS = {**HEADERS, "Accept-Language": "en-US,en;q=0.9", "Content-Type": "application/json",
                "X-Requested-With": "XMLHttpRequest", "Referer": "https://www.fcmetz.com/equipe-premiere/effectif"}
...
def build_metz_official_lookup():
    lookup = {}
    try:
        r = requests.post(METZ_SQUAD_API, json=METZ_SQUAD_MODEL, headers=METZ_HEADERS, timeout=20)
        r.raise_for_status()
        pattern = r"teamSeasonPlayerId=(\d+)&Type=Picture1&pictureName=(\d+)[^']+'\s+.*?alt='picture of @([^']+)'"
        for tid, pic, name in re.findall(pattern, r.text, re.S):
            norm = normalize(name.strip())
            url = (f"{METZ_IMG_BASE}?teamSeasonPlayerId={tid}&Type=Picture1"
                   f"&pictureName={pic}&defaultDocumentPath=wwwroot%2Fimg%2Fgeneral%2Fsilhouette-fullbody.png")
            lookup[norm] = url
        print(f"  FC Metz official entries: {len(lookup)}")
    except Exception as e:
        print(f"  WARNING Metz official failed: {e}")
    return lookup

def metz_official_url_for(name, metz_lookup):
    matched = _squad_match(normalize(name), metz_lookup)
    return metz_lookup[matched] if matched else None

def main():
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    print("=" * 60)
    print("  FC Metz — Player Headshot Scraper")
    print("=" * 60)
    espn_roster   = build_espn_roster()
    tm_lookup     = build_tm_lookup()
    metz_lookup   = build_metz_official_lookup()
    print(f"\n  ESPN: {len(espn_roster)}  TM: {len(tm_lookup)}  Metz: {len(metz_lookup)}  Output: {OUTPUT_DIR}\n" + "-"*60)
    ...
    for i, player in enumerate(espn_roster, 1):
        ...
        url = metz_official_url_for(name, metz_lookup)
        if url and download_image(url, filepath, hdrs=METZ_HEADERS, min_size=20000):
            print(f"           [METZ  ] Saved → {filename}"); downloaded += 1; time.sleep(0.3); continue
