"""
Manchester United FC — Player Headshot Downloader (Soccer / EPL)
...
Sources (tried in order per player):
  1. PL25 CDN      — resources.premierleague.com/premierleague25/110x140 (2025/26 kits)
                     No 'p' prefix. Covers 32 squad members in current Man Utd kit.
  2. FPL CDN       — resources.premierleague.com/premierleague/110x140 (legacy)
                     Falls back for any code not yet on PL25.
  3. Transfermarkt — img.a.transfermarkt.technology portrait photos.
                     Fallback for fringe/youth players not in either PL CDN.
  4. ESPN CDN      — espncdn.com headshots (very sparse, last resort).
...
Squad sourced from:
  https://www.espn.com/soccer/team/squad/_/id/360/manchester-united
"""
...
# ── Config ─────────────────────────────────────────────────────────────────────
ESPN_ROSTER_API  = (
    "https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/teams/360/roster"
)
...
TM_KADER_URL     = "https://www.transfermarkt.com/manchester-united/kader/verein/985"

OUTPUT_DIR = Path("f:/logo/manchester_united_soccer_player_logos")
...
_CHAR_MAP = str.maketrans({
    ...
    "ß": "ss",
    "ł": "l", "Ł": "L",
    "ı": "i", "İ": "I",   # Turkish dotless-i (e.g. Bayındır)
})
...
def build_espn_roster() -> list[dict]:
    players = []
    try:
        print("  Fetching ESPN roster (team 360 / eng.1) ...")
...
def build_fpl_lookup() -> dict:
    """
    Returns {normalized_name: best_photo_url} for every Man Utd player in FPL.
    Prefers PL25 CDN (2025/26 current-kit), falls back to legacy FPL CDN.
    The __fallback key holds the legacy URL for any player not yet on PL25.
    """
    lookup: dict = {}
    try:
...
def main() -> None:
    ...
    print("=" * 60)
    print("  Manchester United FC — Player Headshot Scraper")
    print("=" * 60)
