"""
Everton FC — Player Headshot Downloader (Soccer / EPL)
...
Sources (tried in order per player):
  1. PL25 CDN      — resources.premierleague.com/premierleague25/110x140 (2025/26 kits)
                     No 'p' prefix. Covers 31 squad members in current Everton kit.
  2. FPL CDN       — resources.premierleague.com/premierleague/110x140 (legacy fallback).
  3. Transfermarkt — img.a.transfermarkt.technology portrait photos (25 players).
...
Squad sourced from:
  https://www.espn.com/soccer/team/squad/_/id/368/everton
"""
...
# ── Config ─────────────────────────────────────────────────────────────────────
ESPN_ROSTER_API  = (
    "https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/teams/368/roster"
)
...
TM_KADER_URL     = "https://www.transfermarkt.com/fc-everton/kader/verein/29"

OUTPUT_DIR = Path("f:/logo/everton_soccer_player_logos")
...
def build_espn_roster() -> list[dict]:
    players = []
    try:
        print("  Fetching ESPN roster (team 368 / eng.1) ...")
...
def build_fpl_lookup() -> dict:
    """
    Returns {normalized_name: best_photo_url} for every Everton player in FPL.
...
def main() -> None:
    ...
    print("=" * 60)
    print("  Everton FC — Player Headshot Scraper")
    print("=" * 60)
