...
# Comprehensive list of all cricket leagues
CRICKET_LEAGUES = [
...
def download_league_logos(league_name):
    ...
    try:
        ...
        # Download logos
        logos_folder = f"cricket_logos/{safe_league_name}"
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("🏏 SCRAPING ALL CRICKET LEAGUES")
    print("="*70)
    print(f"\nTotal leagues to scrape: {len(CRICKET_LEAGUES)}")
    print("This will take a few minutes...\n")
    
    # Create summary
    summary = {
        "total_leagues": len(CRICKET_LEAGUES),
        "successful_leagues": 0,
        "total_teams": 0,
        "leagues": []
    }
    
    for i, league in enumerate(CRICKET_LEAGUES, 1):
        print(f"\n[{i}/{len(CRICKET_LEAGUES)}] Processing: {league}")
        ...
        if i < len(CRICKET_LEAGUES):
            time.sleep(1.5)
    
    # Save summary
    with open("cricket_scraping_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)
    
    print("\n" + "="*70)
    print("🎉 CRICKET SCRAPING COMPLETE!")
    print("="*70)
    ...
    print(f"\n📁 Logos saved in: cricket_logos/")
    print(f"📄 Data saved in: cricket_data/")
    print(f"📊 Summary saved: cricket_scraping_summary.json")
    print("="*70 + "\n")
