"""
Burnley FC — Player Headshot Downloader (Soccer / EPL)
...
Sources (tried in order per player):
  1. PL25 CDN      — resources.premierleague.com/premierleague25/110x140 (2025/26 kits)
                     No 'p' prefix. Covers 40 squad members in current Burnley kit.
...
Squad sourced from:
  https://www.espn.com/soccer/team/squad/_/id/379/burnley
"""
...
# ── Config ─────────────────────────────────────────────────────────────────────
ESPN_ROSTER_API  = (
    "https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/teams/379/roster"
)
...
TM_KADER_URL     = "https://www.transfermarkt.com/fc-burnley/kader/verein/1132"

OUTPUT_DIR = Path("f:/logo/burnley_soccer_player_logos")
...
# ESPN sometimes uses only a single/short name; map to FPL full name here.
ESPN_TO_FPL_NAME: dict[str, str] = {
    "Florentino": "Florentino Luis",   # FPL may store full name
}
...
def _squad_match(key: str, roster: dict) -> str | None:
    ...
    for rk in roster:
        ...
        if rw[-1] == last and len(common) >= 1:
            return rk
        # Single-name players (e.g. "Florentino", "Hannibal")
...
def build_espn_roster() -> list[dict]:
    players = []
    try:
        print("  Fetching ESPN roster (team 379 / eng.1) ...")
...
def build_fpl_lookup() -> dict:
    """
    Returns {normalized_name: best_photo_url} for every Burnley player in FPL.
...
def main() -> None:
    ...
    print("=" * 60)
    print("  Burnley FC — Player Headshot Scraper")
    print("=" * 60)
