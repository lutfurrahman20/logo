"""
Brentford FC — Player Headshot Downloader (Soccer / EPL)
...
Sources (tried in order per player):
  1. PL25 CDN      — resources.premierleague.com/premierleague25/110x140 (2025/26 kits)
                     No 'p' prefix. Covers 38 squad members in current Brentford kit.
...
Squad sourced from:
  https://www.espn.com/soccer/team/squad/_/id/337/brentford
"""
...
# ── Config ─────────────────────────────────────────────────────────────────────
ESPN_ROSTER_API  = (
    "https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/teams/337/roster"
)
...
TM_KADER_URL     = "https://www.transfermarkt.com/fc-brentford/kader/verein/1148"

OUTPUT_DIR = Path("f:/logo/brentford_soccer_player_logos")
...
ESPN_TO_FPL_NAME = {
    "Conor Mcmanus":     "Conor McManus",
    "Yehor Yarmoliuk":   "Yehor Yarmoliuk",   # TM spells "Yegor" — ESPN/FPL use "Yehor"
    "Fábio Carvalho":    "Fábio Ferreira Carvalho",
}
...
def build_espn_roster() -> list[dict]:
    players = []
    try:
        print("  Fetching ESPN roster (team 337 / eng.1) ...")
...
def build_fpl_lookup() -> dict:
    """
    Returns {normalized_name: best_photo_url} for every Brentford player in FPL.
    ...
    try:
        ...
        for p in r.json().get("elements", []):
            if p.get("team") != BRENTFORD_FPL_ID:
                continue
            code  = p.get("code", "")
...
def main() -> None:
    ...
    print("=" * 60)
    print("  Brentford FC — Player Headshot Scraper")
    print("=" * 60)
