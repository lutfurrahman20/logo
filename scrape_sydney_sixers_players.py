"""
Sydney Sixers BBL Player Image Downloader
Fetches player images from sydneysixers.com.au
...
OUTPUT_DIR = Path("f:/logo/sydney_sixers_players")
...
HEADERS = {
    ...
    "Referer": "https://www.sydneysixers.com.au/",
    "Accept": "image/avif,image/webp,image/apng,image/*,*/*;q=0.8",
    "Accept-Language": "en-US,en;q=0.9",
}

SOURCE_URL = "https://www.sydneysixers.com.au/bbl-players"
...
def fetch_player_images(html: str) -> dict[str, str]:
    """Extract all (name → image_url) from the page, appending required size params."""
    ...
    for name, block in pod_blocks:
        m = re.search(
            r'srcset="(https://resources\.sydney-sixers\.pulselive\.com/photo-resources/[^?"]+)',
            block,
        )
        if not m:
            m = re.search(
                r'src="(https://resources\.sydney-sixers\.pulselive\.com/photo-resources/[^?"]+)',
                block,
            )
        if m:
...
def main():
    ...
    print("=" * 65)
    print("Sydney Sixers BBL Player Image Downloader")
    print("=" * 65)
    print(f"Fetching: {SOURCE_URL}\n")

    fetch_headers = {k: v for k, v in HEADERS.items() if k != "Accept"}
    resp = requests.get(SOURCE_URL, headers=fetch_headers, timeout=30)
