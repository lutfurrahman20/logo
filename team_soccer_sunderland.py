"""
Sunderland AFC — Player Headshot Downloader (Soccer / Championship)
...
Sources (tried in order per player):
  1. PL25 CDN      — resources.premierleague.com/premierleague25/110x140 (2025/26 kits)
                     No 'p' prefix. Covers ~44 squad members.
...
Squad sourced from:
  https://www.espn.com/soccer/team/squad/_/id/366/sunderland
"""
...
# ── Config ─────────────────────────────────────────────────────────────────────
ESPN_ROSTER_API    = (
    "https://site.api.espn.com/apis/site/v2/sports/soccer/eng.2/teams/366/roster"
)
...
SUNDERLAND_FPL_ID  = 17

TM_KADER_URL       = "https://www.transfermarkt.com/sunderland-afc/kader/verein/289"

OUTPUT_DIR = Path("f:/logo/sunderland_soccer_player_logos")
...
def build_espn_roster() -> list[dict]:
    players = []
    try:
        print("  Fetching ESPN roster (team 366 / eng.2) ...")
...
def build_fpl_lookup() -> dict:
    """
    Returns {normalized_name: best_photo_url} for every Sunderland player in FPL.
    ...
    try:
        ...
        for p in r.json().get("elements", []):
            if p.get("team") != SUNDERLAND_FPL_ID:
                continue
            code  = p.get("code", "")
...
def main() -> None:
    ...
    print("=" * 60)
    print("  Sunderland AFC — Player Headshot Scraper")
    print("=" * 60)
