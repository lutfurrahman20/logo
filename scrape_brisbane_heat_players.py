"""
Brisbane Heat BBL Player Image Downloader
Fetches player images from brisbaneheat.com.au
...
OUTPUT_DIR = Path("f:/logo/brisbane_heat_players")
...
HEADERS = {
    ...
    "Referer": "https://www.brisbaneheat.com.au/",
...
SOURCE_URL = "https://www.brisbaneheat.com.au/players/bbl"
...
def fetch_player_images(html: str) -> dict[str, str]:
    """Extract all (name → image_url) from the page with size params appended."""
    ...
    for name, block in pod_blocks:
        m = re.search(
            r'srcset="(https://resources\.brisbane-heat\.pulselive\.com/photo-resources/[^?"]+)',
            block,
        )
        if not m:
            m = re.search(
                r'src="(https://resources\.brisbane-heat\.pulselive\.com/photo-resources/[^?"]+)',
                block,
            )
        if m:
...
def main():
    ...
    print("=" * 65)
    print("Brisbane Heat BBL Player Image Downloader")
    print("=" * 65)
