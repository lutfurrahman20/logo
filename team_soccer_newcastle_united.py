"""
Newcastle United FC — Player Headshot Downloader (Soccer / EPL)
...
Sources (tried in order per player):
  1. PL25 CDN      — resources.premierleague.com/premierleague25/110x140 (2025/26 kits)
                     No 'p' prefix. Covers 34 squad members in current Newcastle kit.
  2. FPL CDN       — resources.premierleague.com/premierleague/110x140 (legacy fallback).
  3. Transfermarkt — img.a.transfermarkt.technology portrait photos (26 players).
...
Squad sourced from:
  https://www.espn.com/soccer/team/squad/_/id/361/newcastle-united
"""
...
# ── Config ─────────────────────────────────────────────────────────────────────
ESPN_ROSTER_API    = (
    "https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/teams/361/roster"
)
...
TM_KADER_URL       = "https://www.transfermarkt.com/newcastle-united/kader/verein/762"

OUTPUT_DIR = Path("f:/logo/newcastle_united_soccer_player_logos")
...
def build_espn_roster() -> list[dict]:
    players = []
    try:
        print("  Fetching ESPN roster (team 361 / eng.1) ...")
...
def build_fpl_lookup() -> dict:
    """
    Returns {normalized_name: best_photo_url} for every Newcastle player in FPL.
    ...
    try:
        ...
        for p in r.json().get("elements", []):
            if p.get("team") != NEWCASTLE_FPL_ID:
                continue
            code  = p.get("code", "")
...
def main() -> None:
    ...
    print("=" * 60)
    print("  Newcastle United FC — Player Headshot Scraper")
    print("=" * 60)
