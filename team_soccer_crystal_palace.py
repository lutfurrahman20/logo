"""
Crystal Palace FC — Player Headshot Downloader (Soccer / EPL)
...
Sources (tried in order per player):
  1. PL25 CDN      — resources.premierleague.com/premierleague25/110x140 (2025/26 kits)
                     No 'p' prefix. Covers 34 squad members in current Crystal Palace kit.
...
Squad sourced from:
  https://www.espn.com/soccer/team/squad/_/id/384/crystal-palace
"""
...
# ── Config ─────────────────────────────────────────────────────────────────────
ESPN_ROSTER_API   = (
    "https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/teams/384/roster"
)
...
TM_KADER_URL      = "https://www.transfermarkt.com/crystal-palace/kader/verein/873"

OUTPUT_DIR = Path("f:/logo/crystal_palace_soccer_player_logos")
...
ESPN_TO_FPL_NAME: dict[str, str] = {
    # Add overrides if ESPN name differs significantly from FPL name
}
...
def build_espn_roster() -> list[dict]:
    players = []
    try:
        print("  Fetching ESPN roster (team 384 / eng.1) ...")
...
def build_fpl_lookup() -> dict:
    """
    Returns {normalized_name: best_photo_url} for every Crystal Palace player in FPL.
...
def main() -> None:
    ...
    print("=" * 60)
    print("  Crystal Palace FC — Player Headshot Scraper")
    print("=" * 60)
